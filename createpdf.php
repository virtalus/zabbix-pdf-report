...
$report_start	= date('Y-m-d',$starttime);
$report_end	= date('Y-m-d',$endtime);
...
$stringData = "This is a report automatically generated by the HyperCX Monitoring by Zabbix monitoring system, this report contains the information about all the physical hosts. Virtalus is committed to providing all of its clients with a report that shows the performance of all information.\n";
fwrite($fh, $stringData);
$stringData = "#NP\n";
fwrite($fh, $stringData);
#$stringData = "1<Graphs>\n";
...
$pdf->addText(50,34,6,'Virtalus Report.');
$pdf->restoreState();
$pdf->closeObject();
...
$pdf->ezText("$company_name Report",(40-strlen($company_name)),array('justification'=>'centre'));
...
$pdf->ezText("",14,array('justification'=>'centre'));
$pdf->ezText(date('l jS \of F Y'),19,array('justification'=>'centre'));
//$pdf->ezText("(Version " . $version . ")",14,array('justification'=>'centre'));

$pdf->openHere('Fit');

company_logo($pdf,150,$pdf->y-80,80,150,200);
$pdf->addPngFromFile($pdf_logo,10,$pdf->y,10,10);
...
if (file_exists($pdf_logo)){
  ...
  $pdf->addPngFromFile($pdf_logo,199,$pdf->y,200,0);
  if ($debug) { 
...
//  } else if (((strlen($line)>1 && $line[1]=='<') || (strlen($line) && $line[0]=='<')) && $line[strlen($line)-1]=='>') {
    ...
    switch($line[0]){
      ...
      default:
		...
		$pdf->EzImage($image, '40', '420', 'none', 'centre');
  } else {
